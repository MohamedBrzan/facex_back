...
exports.default = (0, AsyncHandler_1.default)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!post)
        return next(new ErrorHandler_1.default(404, `Couldn't Find Post With Id => ${postId}`));
    ...
    const usersWhoReacted = (0, ExpressionLoop_1.default)(post);
    //* Get All Users Who Comments
    // TODO: Comments Loop
    for (let i = 0; i < post.comments.length; i++) {
        const comment = yield Comment_1.default.findById(post.comments[i].toString());
    ...
    for (let i = 0; i < post.shares.length; i++) {
        const user = yield User_1.default.findById(post.shares.toString()).select('shares');
    ...
    for (let userId of usersWhoReacted) {
        const user = yield User_1.default.findById(userId).select('posts');
        user.posts.reacted.splice(user.posts.reacted.indexOf(postId), 1);
        yield user.save();
    }
    //* Delete the Post
    yield Post_1.default.findByIdAndRemove(postId);
    return res
        .status(200)
